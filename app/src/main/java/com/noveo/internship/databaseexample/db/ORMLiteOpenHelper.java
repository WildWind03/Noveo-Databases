...
public class ORMLiteOpenHelper extends OrmLiteSqliteOpenHelper {
    ...
    private RuntimeExceptionDao<Toy, Integer> toyDaoException;
    ...
    public ToyDao getToyDAO() throws SQLException{
        if(toyDao == null){
            toyDao = new ToyDao(getConnectionSource(), Toy.class);
        }
    ...
    public RuntimeExceptionDao<Toy, Integer> getRuntimeExceptionDao() {
        if (toyDaoException == null) {
            toyDaoException = getRuntimeExceptionDao(Toy.class);
        }
        return toyDaoException;
    }
}